...
# Student: Haichuan Xie
#
# DEMO VERSION — pre-loaded with sample data for presentation
# HOW TO RUN:  python app_gui_demo.py
# ============================================================
"""
...
def load_demo_data():
    """Pre-load sample data so the presentation shows a realistic scenario.

    Story:
      MacBook Pro M4  — inflated by a shill (max99) always giving 10/10
      Dell XPS 15     — dragged down by 3 coordinated fake-low accounts
      Surface Laptop 7 — clean, honest reviews only

    The system detects all suspicious users and reduces their influence,
    shown as red rows in the User Weights tab.
    """

    # ── users ────────────────────────────────────────────────────────
    #  alice  — normal reviewer, varied scores  → weight 1.0
    users["alice"]  = {"weight": 1.0, "history": [9.0, 6.0, 7.0], "note": "Normal"}
    #  bob    — normal reviewer, varied scores  → weight 1.0
    users["bob"]    = {"weight": 1.0, "history": [8.0, 5.5, 7.0], "note": "Normal"}
    #  carol  — scores too similar (low variance) → weight 0.7
    users["carol"]  = {"weight": 0.7, "history": [8.0, 7.5],       "note": "Low score variety - influence slightly reduced"}
    #  max99  — always 10/10 → weight 0.4  (shill boosting MacBook)
    users["max99"]  = {"weight": 0.4, "history": [10.0, 10.0, 10.0], "note": "Always high scores - influence reduced"}
    #  shills — always 1/10 → weight 0.4  (fake-low attacking Dell)
    users["shill_a"] = {"weight": 0.4, "history": [1.0, 1.0], "note": "Always low scores - influence reduced"}
    users["shill_b"] = {"weight": 0.4, "history": [1.0, 1.0], "note": "Always low scores - influence reduced"}
    users["shill_c"] = {"weight": 0.4, "history": [1.0, 1.0], "note": "Always low scores - influence reduced"}

    # ── items ─────────────────────────────────────────────────────────
    items["MacBook Pro M4"] = {
        "reviews": [
            {"user": "alice", "raw": 9.0,  "satisfaction": 0.9},
            {"user": "bob",   "raw": 8.0,  "satisfaction": 0.8},
            {"user": "carol", "raw": 8.0,  "satisfaction": 0.85},
            # max99 spams three identical 10/10 reviews
            {"user": "max99", "raw": 10.0, "satisfaction": 1.0},
            {"user": "max99", "raw": 10.0, "satisfaction": 1.0},
            {"user": "max99", "raw": 10.0, "satisfaction": 1.0},
        ],
        "score": 0.0,
    ...
    items["Dell XPS 15"] = {
        "reviews": [
            {"user": "alice",   "raw": 6.0, "satisfaction": 0.8},
            {"user": "bob",     "raw": 5.5, "satisfaction": 0.7},
            # three shill accounts flood with fake 1/10 reviews
            {"user": "shill_a", "raw": 1.0, "satisfaction": 1.0},
            {"user": "shill_a", "raw": 1.0, "satisfaction": 1.0},
            {"user": "shill_b", "raw": 1.0, "satisfaction": 1.0},
            {"user": "shill_b", "raw": 1.0, "satisfaction": 1.0},
            {"user": "shill_c", "raw": 1.0, "satisfaction": 1.0},
            {"user": "shill_c", "raw": 1.0, "satisfaction": 1.0},
        ],
        "score": 0.0,
    }
    items["Surface Laptop 7"] = {
        "reviews": [
            {"user": "carol", "raw": 7.5, "satisfaction": 0.85},
            {"user": "bob",   "raw": 7.0, "satisfaction": 0.75},
            {"user": "alice", "raw": 7.0, "satisfaction": 0.7},
        ],
        "score": 0.0,
    }
...
if __name__ == "__main__":
    load_demo_data()           # <-- pre-load sample data
    root = tk.Tk()
    app = SmartReviewApp(root)
    app._refresh_add_tree()    # show items in Add tab immediately
    app._refresh_rate_combo()  # populate item dropdown
    root.mainloop()
